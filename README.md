...
- NASDK完全没有设计鉴权系统，我们在设计时就认为，所有连接进来的 App 都可信。因此，请不要直接把通讯入口公开在公网上！（接下来的版本可能会对此提供基本加密，或者提供插件形式进行拦截，但是**用户身份识别**相关是一定不会做的，只会做鉴权和**App可信**。）
- NASDK/NACP从来没有设计为持久化消息队列形式，虽然NASDK提供了有限的消息队列缓存和中继支持，但不保证断线重连能否有效，只能尽力而为传输消息（NACP最大的缺陷是没有消息接收确认，即Notify和Response应该额外响应一个包表示确认接收，这个问题也有可能在接下来的版本改进）。
- NACEB暂时没有设计为分布式协调，他不能自动协调两个NApp的资源占用。（这一部分问题将在接下来的版本中解决）
- NACAB和NACEB的AsyncTask被设计为绝对能够并发启动，没有设置上限。（这个问题是刻意为之，需要开发者自行解决）
